...
class Question {
    ...
    appendQuestion(parent, qNum, round, timerId, qMax) { // appends the question (and its answers) to a parent wrapper
        this.parent = parent;
        this.qElement = document.createElement("p");
        this.qElement.innerText = "Question "+qNum+" of "+qMax+": "+this.htmlDecode(this.question);
        ...
        for(let i = 0; i < this.answers.length; i++) {
            ...
            this.aElements[i].innerText = this.htmlDecode(this.answers[i]);
            this.aElements[i].classList.add("answer");
            this.aElements[i].setAttribute("data-answer", this.answers[i]);
            this.aElements[i].addEventListener("mouseup", (e) => {
                e.preventDefault();
                console.log(e.target.dataset.answer + " " + this.answer)
                if(e.target.dataset.answer === this.answer) {
...
class Round {
    ...
    incrementScore(rightOrWrong) {
        if(rightOrWrong) {
            this.roundScore += parseInt(this.timer.innerText, 10);
            this.rightCount++;
        } else {
            if(this.roundScore - 3 < 0) {
                this.roundScore = 0;
            } else {
                this.roundScore -= 2;
            }
    ...
    createScoreBox() {
        ...
        this.createTimer();
        timerBox.appendChild(this.timer);
        scoreBox.appendChild(timerBox);
    ...
    displayQuestion() { // displays the question in the parent according to round count
        this.createScoreBox();
        this.questions[this.roundCount].appendQuestion(this.parent, this.roundCount + 1, this, this.timerId, this.roundLength);
    ...
    roundFinish() { // ends the round and displays landing screen for the end of a round
        ...
        let roundEndText1 = document.createElement("p");
        let roundEndText2 = document.createElement("p");
        let roundEndText3 = document.createElement("p");
        roundEndText1.innerText = "All "+this.roundLength+" questions have been answered.";
        roundEndText2.innerText = "You scored "+this.roundScore+" points.";
        roundEndText3.innerText = "You got "+this.rightCount+" questions right and "+ this.wrongCount+" questions wrong.";
        roundEndText1.classList.add("roundend");
        roundEndText2.classList.add("roundend");
        roundEndText3.classList.add("roundend");
        roundEnd.classList.add("roundend-box");
        roundEnd.appendChild(roundEndText1)
        roundEnd.appendChild(roundEndText2)
        roundEnd.appendChild(roundEndText3)
...
class Game {
    ...
    createMainMenu() {
        ...
        greeting.classList.add("main-menu-greeting");
        ...
        questionNumField.setAttribute("placeholder", "Number of Questions (1-50)");
        ...
        fetch("https://opentdb.com/api_category.php")
            ...
            .then(res => {
                ...
                catagoryDropDown.classList.add("form-element");
                difficultyDropDown.classList.add("form-element");
                questionNumField.classList.add("form-element");
        ...
        directions.innerText = "Instructions: Once you start you will have 15 seconds to answer the question.\nIf you get it correct the remaining time will be added to your score. If you get it wrong, however, you will lose 2 points.\nHave fun and try and get as high as you can!"
        directions.classList.add("directions");
        this.gameParent.appendChild(directions);

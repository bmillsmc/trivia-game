...
class Round {
    ...
    createTimer() {
        ...
        } else if(currTime.getHour() + 1 > 23) {
            timeLimit = new Date(currTime.getFullYear(), currTime.getMonth(), currTime.getDay() + 1);
        } else if(currTime.getMinute() + 1 > 59) {
            timeLimit = new Date(currTime.getFullYear(), currTime.getMonth(), currTime.getDay(), currTime.getHour() + 1);
        } else {
            timeLimit = new Date(currTime.getFullYear(), currTime.getMonth(), currTime.getDay(), currTime.getHour(), currTime.getMinute() + 1);
        }
        let now = currTime.getTime();
        let later = timeLimit.getTime();
        let time = later - now;

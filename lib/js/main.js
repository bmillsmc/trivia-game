...
class Question {
    ...
    appendQuestion(parent) { // appends the question (and its answers) to a parent wrapper
        ...
        parent.appendChild(this.qElement);
        this.aElements;
        for(let i = 0; i < this.answers.length; i++) {
            ...
            this.aElements[i].addEventListener("click", (e) => {
                ...
                if(e.target.innerText === this.answer) {
                    ...
                    this.parent.insertBefore(correct, this.aElements[0]);
                } else {
                    let incorrect = document.createElement("p");
                    incorrect.innerText = "Incorrect";
                    incorrect.classList.add("incorrect");
                    this.parent.insertBefore(incorrect, this.aElements[0]);
                }
                this.showAnswer();
            })
            parent.appendChild(this.aElements[i]);
        }
    }
...
class Round {
    ...
    displayQuestion() { // displays the question in the parent according to round count
        this.questions[this.roundCount].appendQuestion(this.parent);
    }
    ...
    roundFinish() { // ends the round and displays landing screen for the end of a round
        ...
        roundEnd.innerText = "All "+this.roundLength+" have been answered. The round is over.";
    ...
    eraseParent() { // makes sure the parent div is blank and if not erases whats in it
        for(let i = 0; i < this.parent.childNodes.length; i++) {
            // console.log("erase "+this.parent.childNodes[i]);
...
let q1 = new Question("what is beemooo", ["nock", "nilk", "nork"], "nick");
let q2 = new Question("wut be boysssss", ["boys", "boyz", "boyzzzzz"], "boysssss");

let qArray = [q1, q2];

let round1 = new Round(qArray, triviaGame);

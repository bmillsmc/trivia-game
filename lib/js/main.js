...
class Game {
    ...
    createMainMenu() {
        ...
        fetch("https://opentdb.com/api_category.php")
            ...
            .then(res => {
                console.dir(res);
                catagories = res.trivia_catagories;
            })
            .catch(err => {
                console.log("error",err);
            })
        let catagoryDropDown = this.createDropDown(catagories, "Catagory");
        let difficultyDropDown = this.createDropDown(difficulties, "Difficulty");
        let questionNumField = document.createElement("input");
        questionNumField.setAttribute("type", "text");
    ...
    createDropDown(options, title) { // takes an array of options and a title, returns a select wrapper with options appended and default as any catagory
        ...
        for(let i = 1; i < options.length + 1; i++) {
            ...
            if(i === 0) {
                optionsEl[i].setAttribute("value", "");
